...
A step-by-step flowchart of `install.sh` — what it keeps vs overrides, plus the
`setup.sh` branch on a fresh server — is in the README:
**[The complete flow](README.md#the-complete-flow)**.

The manual steps below are that same flow, one command per piece.
